...
int set_gpio_direction(std::string gpio, gpio_direction direction)
{
  ...
  snprintf(buffer, sizeof(buffer), SYSFS_GPIO_DIR "/%s/direction", gpio.c_str());
...
int set_gpio_state(std::string gpio, gpio_state state)
{
  ...
  snprintf(buffer, sizeof(buffer), SYSFS_GPIO_DIR "/%s/value", gpio.c_str());
...
{
  return export_gpio(pin_gpio_mapping[gpio_pin].first);
}

int unexport_gpio_pin(int gpio_pin)
{
  return unexport_gpio(pin_gpio_mapping[gpio_pin].first);
}

int set_gpio_pin_direction(int gpio_pin, gpio_direction direction)
{
  return set_gpio_direction(pin_gpio_mapping[gpio_pin].second, direction);
}

int set_gpio_pin_state(int gpio_pin, gpio_state state)
{
  return set_gpio_state(pin_gpio_mapping[gpio_pin].second, state);
}
